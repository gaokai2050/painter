...
BOOL QUPointInLine(Line *line, CGPoint point, BOOL includeEndpoint)
{
    ...
    if (QUIsSamePoint(line.start, point) || QUIsSamePoint(line.end, point)) {
        return includeEndpoint;
    }
    double x = (point.x - line.start.x) * (line.end.y - line.start.y) - (point.y - line.start.y) * (line.end.x - line.start.x);
    //如果两个向量不共线，返回NO。
    if (x > QU_INACCURACY_DELTA) {
        return NO;
    }
    //返回是否在线段内
    return QUIsSameDirection(line.start, point, line.end);
}
//求两个线段的交点。
//如果两条线平行：
